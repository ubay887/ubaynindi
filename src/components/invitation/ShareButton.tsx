...
export function ShareButton() {
  ...
  const copyLink = async () => {
    const ok = await copyToClipboard(getInviteUrl({ code, side }));
    if (ok) {
      setCopied(true);
  ...
  return (
    <div className="fixed bottom-5 left-5 z-40">
      {open ? (
        <motion.div
          className="mb-2 flex flex-col gap-1.5 rounded-2xl border border-white/50 bg-cream/95 p-2 shadow-[0_14px_36px_-14px_rgba(46,63,44,0.4)] backdrop-blur-md"
        ...
        >
          <button
            type="button"
            onClick={shareWa}
            className="flex items-center gap-2 rounded-xl px-3 py-2 text-left text-xs font-medium text-primary-dark hover:bg-primary/8"
          >
            <span className="flex h-7 w-7 items-center justify-center rounded-full bg-[#25D366]/15 text-[#128C7E]">
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-xl px-3 py-2 text-left text-xs font-medium text-primary-dark hover:bg-primary/8"
          >
            <span className="flex h-7 w-7 items-center justify-center rounded-full bg-primary/10 text-primary-dark">
      ...
      <motion.button
        ...
        aria-expanded={open}
        className="flex h-11 w-11 items-center justify-center rounded-full border-2 border-primary-dark/15 bg-cream/95 text-primary-dark shadow-[0_12px_30px_-12px_rgba(46,63,44,0.45)] backdrop-blur-md"
        whileTap={{ scale: 0.94 }}
